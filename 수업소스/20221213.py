# -*- coding: utf-8 -*-
"""
Created on Tue Dec 13 09:28:41 2022
...
'''
    한번의 주문 시 주문금액이 50달러 이상인 주문의 id 출력하기
    주문번호    주문금액
       1          25
       1          30
       2          2
...
beer_wine_corr = drinks[["beer_servings","wine_servings"]]\
    .corr(method="kendall")
beer_wine_corr
# 스피어만 상관계수 : 정규화가 되지 않은 데이터에 많이 사용
beer_wine_corr = drinks[["beer_servings","wine_servings"]]\
...
# 회귀그래프 작성하기
sns.regplot(x="beer_servings",y="total_litres_of_pure_alcohol",
            data=drinks)
...
# 대륙별 알콜양의 평균이 전체 알콜량 평균보다 많은 대륙을 조회하기
# 전체 알콜 평균
total_mean = drinks["total_litres_of_pure_alcohol"].mean()
total_mean
# 대륙별 알콜 평균
cont_mean = drinks.groupby("continent").mean()["total_litres_of_pure_alcohol"]
cont_mean
# 대륙 조회
...
drinks.groupby("continent").mean()["beer_servings"].idxmin()
...
bar_list[len(continents) - 1].set_color('r')  #7번째 막대 색상을 red로 설정
# plt.plot : 선그래프
# [0.,6] : x축 값
# [total_mean, total_mean] : y축 값
# "k-" : 검정 실선. -- : 점선
plt.plot([0.,6], [total_mean, total_mean], "k-")
# x축의 값을 변경 : 0~6 숫자를 continents의 내용 변경
