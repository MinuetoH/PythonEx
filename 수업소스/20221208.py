# -*- coding: utf-8 -*-
"""
Created on Thu Dec  8 09:04:31 2022
...
for h,n in zip(homelist, namelist) :
    #h : 그래프로 출력할 데이터
    #n : 동의 이름
    plt.plot(h,label=n)  #하니의 그래프에 여러 개의 선그래프 작
plt.legend()  #범례 출력.

#age.csv 파일을 이용하여 선택한 지역의 인구구조와 가장 비슷한
# 인구구조를 가진 지역의 그래프와 지역 출력하기
# 가장 비슷한 지역 한 개만 그래프로 출력
#data : 리스트 객체
name = '역삼1동'  #동명을 정확하게
for row in data :
    if name in row[0] :  #row[0] 문자열에 name 포함?
        # , 를 제거. : 숫자내부의 , 제거
        row = list(map((lambda x:x.replace(",","")),row))
        # np.array(row[3:],dtype=int) : 0세 이후 데이터를 배열 생성
#                                      요소는 int형으로
        # int(row[2] : 정수형 총인구수       
        # home : 0세 이후 인구수를 정수형배열 / 총인구수
...
for row in data :
    ...
    # name의 동과 다른 지역의 데이터의 차의 제곱의 합.
    # s값이 가장 작은 지역이 name 동과 가장 비슷한 인구구조 지
    s = np.sum((home - away) ** 2)
    # s < mn : 다른 지역의 오차합이 더 작은 지역
...
'''
    age.csv 파일 cp949 형태의 파일임. ANSI형태. EUC-KR, KSC5601.
        csv 모듈, open 함수를 이용한 경우, 기본 인코딩이 cp949임.
        MAC은 기본 인코딩방식이 UTF-8임.
    pandas 모듈에서는 기본 인코딩방식이 UTF-8임.
    
    thousands="," : 숫자에 세자리마다 , 를 제거함. 숫자만 읽기,
                    , 를 제거하고 숫자로 읽기
...
df.columns = col_name
df.columns
#df의 모든 컬럼드을 총인구수로 나누기. 비율로 저장하기
...
#지정한 지역과 가장 비슷한 인구구조를 갖는 지역 찾아 그래프로 출력하기
name = "역삼1동"
# df.index : 행정구역명
# df.index.str : 인덱스의 이름을 문자열로 변경
# df.index.str.contain() : 선택된 이름을 포함?
...
#####################################################
# 데이터 전처리 : 원본데이터르ㄹ 원하는 형태로 변경하는 과정
...
#결측값이 아닌 갯수 조회
df.notnull().sum()  #컬럼별 결측값 갯수
df.notnull().sum(axis=0)  #컬럼별 결측값 갯수
df.notnull().sum(axis=1)  #행별 결측값 갯수
#dropna : 결측값 제거
...
most_freq 
# value_counts() 함수 결과의 가장 큰 값의 인덱스
...
df["embark_town"].fillna(most_freq,inplace=True)  #결측값 변경
...
df["embarked"][826:831] #829 : Q
# 앞의 데이터로 치환하기
# method="ffill" : 앞의 데이터로 치환
#        "bfill" / "backfill" : 뒤의 데이터로 치환
