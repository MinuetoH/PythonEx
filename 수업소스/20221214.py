# -*- coding: utf-8 -*-
"""
Created on Wed Dec 14 09:17:53 2022
...
drinks.info()
# continenet 컬럼의 데이터가 결측값인 'OT'으로 치환
...
# South Korea 막대의 색을 red로 변경
bar_list[country_list.index("South Korea")].set_color('r')
plt.ylabel('alcohol rate')
plt.title('liquor drink rank by country')
# xy축범위:[x축값의시작,x축값의종료,y축값의시작,y축값의종료]
...
plt.title('drink servins rank by country')
plt.axis([0, 200, 0, 700])  
# korea_serving_rate : 대한민국 전체 술소비량 데이터. y축 값
...
import numpy as np
'''
    np.polyfit(x값,y값,차수) : x,y 모든 점과의 차이가 
                              가장 적은 직선의 값을 리턴
...
fp1
f1 = np.poly1d(fp1)  #함수. fp1 상수값에 맞는 함수
...
# police_Seoul 데이터의 경찰서 컬럼의 내용을 XX서로 이름을 변경하여
# 관서명 컬럼으로 생성하기
police_Seoul.info()
police_Seoul["관서명"] = \
