# -*- coding: utf-8 -*-
"""
Created on Thu Dec 15 09:09:52 2022
...
data_result["오차"] = \
    np.abs(data_result["절도검거율"]-f1(data_result["소계"]))
...
for n in range(10) : #10개의 구 정보만 출력.
    plt.text(df_sort.iloc[n,]['소계']*1.001,        #x축 값
             df_sort.iloc[n,]['절도검거율']*0.999,    #y축 값
...
f1 = np.poly1d(fp1)
data_result["오차"] = \
    np.abs(data_result["발생"]-f1(data_result["소계"]))
data_result["오차"]
# 오차데이터 기준 내림차순 정렬
df_sort = data_result.sort_values(by="오차", ascending=False)
df_sort
...
plt.plot(fx, f1(fx), ls='dashed', lw=3, color='g') #회귀선그래프
for n in range(10) : #10개의 구 정보만 출력.
    plt.text(df_sort.iloc[n,]['소계']*1.02,        #x축 값
             df_sort.iloc[n,]['발생']*0.997,    #y축 값
...
df.info()
# 1. horsepower 컬럼의 자료형을 float형으로 변경하기
# ?를 결측값 변경, 결측값 행을 삭제,변경
...
'''
    ...
    test_size=0.3 : 훈련:데스트=7:3, 기본값 : -.25
    seed 설정 : 데이터의 복원을 위한 설정
'''
from sklearn.model_selection import train_test_split
X_train,X_test,Y_train,Y_test = \
    train_test_split(X,Y,test_size=0.3,random_state=10)
len(X_train)  #392 * 0.7 = 274.4 => 274 : 훈련데이트 독립변수
...
r_square  #결정계수. 값이 작을수록 성능이 좋다. 0.6822458558299325
r_square = lr.score(X,Y)
r_square  # 06919759765239987
# 전체 데이터 평가하기
...
'''
    단순회귀분석 : 독립변수, 종속변수가 한 개인 경우
        단항 : 1차함수
        다항 : 다차원함수
    다중회귀분석 : 독립변수가 여러 개, 종속변수는 한 개
        Y=a1X1 + a2X2 + ... +anXn + b
'''
...
seoul.head()
# seoul1215 변수에 매년 12-15일 날짜만 저장하기
...
del seoul1215["지점"]
# 컬럼명 변경
seoul1215.columns=["날짜","평균기온","최저기온","최고기온","년도","월일"]
seoul1215.info()
# 최저기온이 결측값인 데이터 조회하기
...
# seoul1215 변수에 매년 12-15일 날짜만 저장하기
seoul.info()
seoul0222 = seoul[seoul["월일"]=="02-22"]
seoul0222.info()
del seoul0222["지점"]
seoul0222 = seoul0222.set_index("년도")
seoul0222.info()
seoul0222 = seoul1215.dropna(subset=["최저기온"],axis=0)
seoul0222.plot()
