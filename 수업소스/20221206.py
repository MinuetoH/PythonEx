# -*- coding: utf-8 -*-
"""
Created on Tue Dec  6 09:42:40 2022
...
'''
    pandas 함수
      info() : 기본 정보.
      unique() : 중복없이 한 개의 데이터만 조회
      value_counts() : 데이터별 등록된 건수. 
                       건수의 내림차순으로 정렬
      groupby(컬럼명) : 컬럼의 값으로 레코드를 그룹화. 그룹별 통계자료
    seaborn 모듈을 이용한 그래프
      replot : 산점도 + 회귀선
'''
...
'''
    ...
    violinplot : 값의 범주+분포를 표시. 가로길이 넚은부분은 많은 수치의미
'''
...
seoul_map.save("library2.html")

#MarkerCluster 기능 : 
#               지도 확대 정도에 따라 마커 표시방법을 달리해줌.
#               그룹화 기능
...
'''
    ...
    _ : 변수명. 반복문에서 사용되지 않으므로 상징적으로 변수로 설정
        인덱스 값
    row : 한 개의 레코드
'''
...
library_map.add_child(mc)  #cluster를 지도에 추가
...
folium.Choropleth(geo_data=geo_data,  #위치 정보를 가진 딕셔너리 데이터
    ...
    #데이터와 색상 표시 시 사용되는 범위 지정
    threshold_scale=\
      [10000,100000,200000,300000,400000,500000,600000,700000],
    key_on='feature.properties.name',  #데이터와 지역구분 연결 값
   ).add_to(g_map)
g_map.save('gyoungil_'+ year + '.html')
