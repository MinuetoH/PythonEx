# -*- coding: utf-8 -*-
"""
Created on Fri Dec  2 09:36:03 2022
...
pandas 모듈
 ...
 - DataFrame : 표형태의 데이터처리. Series데이터의 모임.
     ...
     - 얕은복사 : df2 = df df,df2 객체는 물리적으로 같은 객체
...
#value_counts() : 값의 갯수. 내림차순 정렬하여 조회
...
#컬럼 : 변수,피처 용어사용.
# 상관계수 : -1 ~ 1 사이의 값. 변수의 상관관계를 수치로 표현
titanic.corr()
...
'''
    ...
    2. Velocity(속도) : 데이터의 처리 속도가 빨랴야 한다.
...
'''
    상관계수 : -1 ~ 1 사이의 값을 가짐
        컬럼 값의 관계를 수치로 표현
        1 : 상관도 일치. c1 1증가, c2 1증가
        -1 : 상관도 반비례. c1 1증가, c2 -1증가
        0 : 상관이 없다로 판단
    
'''
#mpg 데이터의 컬럼 간의 상관계수 조회하기
mpg.corr()
#mpg, weight 데이터의 컬럼 간의 상관계수 조회하기
mpg[["mpg","weight"]].corr()

#시각화 하기
#연비와 차량의 무게의 관계를 시각화 하기
'''
    산점도 : 두 개 컬럼의 각가의 값들을 x,y축에 점으로 표현
             값의 분포를 알 수 있다.
             컬럼 사이의 관계를 시각화 한다.
'''
...
ndf2.plot()  #범례: 컬럼명
#막대그래프로 출력하기
ndf2.plot(kind="bar")  #범례: 컬럼명. 컬럼 한개 : 막대 한개
ndf2.info()
#데이터 값을 정수형(숫자형)변환
